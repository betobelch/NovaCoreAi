...
export default function LoginPage() {
  ...
  return (
    <main
      ...
      className="login-os-shell relative isolate min-h-screen overflow-hidden px-4 pb-10 pt-28 text-slate-950 sm:px-6 lg:px-8"
    >
      ...
      >
        <motion.section variants={fadeUp} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }}>
          <div className="login-card group relative overflow-hidden rounded-[2rem] border border-white/[0.55] bg-white/[0.58] p-5 shadow-[0_34px_120px_rgba(37,99,235,0.20)] backdrop-blur-2xl sm:p-8 lg:p-9">
            <div className="relative z-10">
              <div className="mb-8 flex items-start gap-4">
                ...
                <div className="min-w-0">
                  <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-blue-400/25 bg-white/[0.52] px-3 py-1.5 text-[0.68rem] font-black uppercase tracking-[0.16em] text-blue-700 shadow-[0_10px_32px_rgba(37,99,235,0.10)] backdrop-blur">
                    <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_16px_rgba(52,211,153,0.95)]" />
                    Core OS seguro
                  </div>
                  <h1 className="max-w-md text-3xl font-black leading-tight text-slate-950 sm:text-4xl">
              ...
              <div className="mb-6 space-y-4">
                <GoogleAuthButton
                  ...
                  className="login-google-button min-h-12 rounded-2xl border-white/60 bg-white/[0.58] text-slate-800 shadow-[0_16px_44px_rgba(15,23,42,0.08)] backdrop-blur-xl hover:border-blue-300/[0.70] hover:bg-white/[0.78] hover:text-slate-950"
                />
                <div className="flex items-center gap-3 text-xs font-bold uppercase tracking-[0.14em] text-slate-400">
                  <span className="h-px flex-1 bg-gradient-to-r from-transparent via-blue-300/80 to-violet-300/80" />
                  <span>acesso por e-mail</span>
                  <span className="h-px flex-1 bg-gradient-to-r from-violet-300/80 via-blue-300/80 to-transparent" />
                </div>
              </div>

              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  <label htmlFor="login-email" className="block text-xs font-black uppercase tracking-[0.14em] text-slate-500">
                    E-mail corporativo
                  </label>
                  <div className="group/input relative">
                    <Mail className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400 transition-colors group-focus-within/input:text-blue-600" />
                    <input
                      id="login-email"
                      type="email"
                      value={email}
                      ...
                      className="login-input min-h-14 w-full rounded-2xl border border-white/60 bg-white/50 px-4 py-3 pl-12 text-sm font-semibold text-slate-900 shadow-[inset_0_1px_0_rgba(255,255,255,0.72)] outline-none transition-all placeholder:text-slate-400 focus:border-blue-400 focus:bg-white/[0.72] focus:ring-4 focus:ring-blue-400/[0.18]"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    <label htmlFor="login-password" className="block text-xs font-black uppercase tracking-[0.14em] text-slate-500">
                      Senha neural
                    </label>
                    <Link
                      href="/login/recuperar-senha"
                      className="text-sm font-black text-blue-700 transition-colors hover:text-violet-700"
                    >
                      Esqueci minha senha
                    </Link>
                  </div>
                  <div className="group/input relative">
                    <LockKeyhole className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400 transition-colors group-focus-within/input:text-blue-600" />
                    <input
                      id="login-password"
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 inline-flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-xl text-slate-500 transition-all hover:bg-blue-500/10 hover:text-blue-700 focus:outline-none focus:ring-3 focus:ring-blue-400/30"
                    >
                  ...
                  <label className="flex items-center gap-3 text-sm font-semibold text-slate-600">
                    <span className="relative flex h-5 w-5 items-center justify-center">
                      <input
                        ...
                        className="peer h-5 w-5 appearance-none rounded-md border border-blue-300/[0.70] bg-white/[0.52] shadow-inner transition-all checked:border-blue-600 checked:bg-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-400/[0.18]"
                      />
                      <CheckCircle2 className="pointer-events-none absolute h-3.5 w-3.5 scale-50 text-white opacity-0 transition-all peer-checked:scale-100 peer-checked:opacity-100" />
                    </span>
                    Lembrar sessão
                  </label>
                  <span className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-[0.13em] text-emerald-600">
                ...
                {status && (
                  <p
                    ...
                    className={`rounded-2xl border px-4 py-3 text-sm font-bold ${
                      status.type === "success"
                        ? "border-emerald-300/60 bg-emerald-400/[0.12] text-emerald-700"
                        : "border-red-300/[0.70] bg-red-400/10 text-red-700"
                    }`}
                  >
                    {status.message}
                  </p>
        ...
        >
          <div className="login-side-panel relative overflow-hidden rounded-[2rem] border border-white/[0.45] bg-white/40 p-6 shadow-[0_38px_130px_rgba(37,99,235,0.18)] backdrop-blur-2xl xl:p-7">
            <div className="relative z-10">
              <div className="mb-7 flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs font-black uppercase tracking-[0.16em] text-blue-700">NovaCore Command</p>
                    <h2 className="text-xl font-black text-slate-950">Painel operacional IA</h2>
                  </div>
                </div>
                ...
                {commandMetrics.map((metric, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <strong className="block truncate text-sm font-black text-slate-950">{metric.label}</strong>
                        <span className="text-xs font-bold text-slate-500">{metric.detail}</span>
                      </div>
                      <span className="text-2xl font-black tracking-tight text-slate-950">{metric.value}</span>
                    </motion.div>
                  )
                })}
              </div>
              ...
              <div className="relative rounded-2xl border border-white/[0.42] bg-white/[0.36] p-5">
                <div className="mb-5 flex items-center justify-between gap-4">
                  <div>
                    <span className="text-xs font-black uppercase tracking-[0.16em] text-violet-700">Fluxo visual</span>
                    <h3 className="mt-1 text-lg font-black text-slate-950">Automação conectada</h3>
                  </div>
                ...
                <div className="relative grid grid-cols-4 gap-3">
                  ...
                  {automationFlow.map((node, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl border border-white/60 bg-white/[0.72] text-blue-700 shadow-[0_14px_34px_rgba(37,99,235,0.12)]">
                          <Icon className="h-5 w-5" />
                        </div>
                        <strong className="mt-3 block text-xs font-black uppercase tracking-[0.08em] text-slate-500">
                          {node.label}
                        </strong>
                        <span className="mt-1 block text-sm font-black text-slate-950">{node.value}</span>
                      </motion.div>
                    )
                  })}
                </div>
